...
while product_name != "Стоп" do
  ...
  product_price_amount_hash[product_price] = product_amount #формирую хэш [цена - количество]
  product_name_hash[product_name] = product_price_amount_hash #формирую хэш [Название товара - хэш [цена - количетсво]]
  product_price_amount_hash = {} #очищаю хэш со значениями [цена - количество] для того, чтобы по следующему товару хэш [цена - количетсво]] заполнялся заново
  end
  #если введенное значение в product_name является "Стоп", то передаем по данной переменной данное значение циклу while для проверки условия
end
